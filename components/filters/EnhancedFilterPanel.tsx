    ...
    <div className="bg-white rounded-lg shadow-sm p-2.5 space-y-2">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => {
            const viewMode = e.target.value as any
            // When switching to geography-mode, clear segments to show Level 1 totals
            if (viewMode === 'geography-mode') {
              updateFilters({ viewMode, segments: [], advancedSegments: [] })
              setSelectedSegments([])
            } else {
